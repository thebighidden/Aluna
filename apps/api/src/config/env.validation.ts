...
export const envValidationSchema = Joi.object({
  ...
  GEMINI_VISION_MODEL: Joi.string().trim().default('gemini-3.6-flash'),
  GEMINI_VISION_INPUT_COST_PER_MILLION_USD: Joi.number().min(0).default(0.3),
  GEMINI_VISION_OUTPUT_COST_PER_MILLION_USD: Joi.number().min(0).default(2.5),
  ...
  GENERATION_DAILY_SPEND_USD: Joi.number().min(0).default(0),
  PUBLIC_APP_URL: Joi.string().trim().default('http://localhost:3000'),
  RESEND_API_KEY: Joi.string().trim().allow('').optional(),
  MAIL_FROM: Joi.string().trim().default('Aluna <onboarding@resend.dev>'),
